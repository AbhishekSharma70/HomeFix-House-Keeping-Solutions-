<template>
    ...
    <nav class="navbar navbar-expand-lg mb-4 shadow-sm bg-primary text-white">
        <div class="container-fluid">
          <a class="navbar-brand text-white" @click="goToProfessionalDashboard">HomeFix | Service Requests</a>
        </div>
              ...
              <table class="table table-hover table-bordered">
                ...
                <tbody>
                  <tr v-for="request in completedServiceRequests" :key="request.id">
                    ...
                    <td><button class="btn btn-info" @click="openUpdateModal(request)"
                      :disabled="request.is_complaint">
                    {{ request.is_complaint ? 'Rated' : 'Rate Customer' }}</button></td>
                  </tr>
                </tbody>
              </table>
            </div>
      ...
      export default {
        ...
        async mounted() {
          ...
          await this.fetchCompletedRequests(professionalId);
        },
        methods: {
          logout() {
      ...
      this.$router.push({ name: 'IndexFirst' });
          ...
          async fetchCompletedRequests(professionalId) {
            ...
            const response = await fetch(
              `http://localhost:8000/api/professional/service_requests/completed/${professionalId}`,{
                headers:{
        ...
        async UpdateServiceRequest() {
          ...
          try {
            const response = await fetch(`http://localhost:8000/api/service_requests_report_customer/${requestId}/update`, {
            method: "PUT",
