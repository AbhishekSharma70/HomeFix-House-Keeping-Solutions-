<template>
  ...
  export default {
    ...
    async mounted() {
      ...
      await this.fetchCompletedPackages(customerId);
    },
    methods: {
      logout(){
      ...
      async fetchCompletedPackages(customerId) {
        ...
        const response = await fetch(`http://localhost:8000/api/booked_service_packages/completed/${customerId}`,{
          'headers':{
